...
export default function Mentor() {
  ...
  const istegiKabul = async (id: number) => {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <p style={{ color: '#888', marginBottom: '2rem' }}>3+ yıl deneyimli arıcılardan mentorluk al</p>
      ...
      {bekleyenIstekler.length > 0 && (
        <div style={{ background: '#1a1a1a', border: '1px solid #E8960A', borderRadius: '12px', padding: '1.5rem', marginBottom: '2rem' }}>
          <h3 style={{ color: '#E8960A', marginBottom: '1rem' }}>📬 Gelen Mentor İstekleri</h3>
          {bekleyenIstekler.map((istek, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              ...
              <button onClick={() => istegiKabul(istek.id)} style={{ background: '#4CAF50', border: 'none', borderRadius: '8px', padding: '0.4rem 1rem', cursor: 'pointer', color: 'white', fontWeight: 'bold' }}>
                Kabul Et
              </button>
            </div>
          ))}
          ...
          return (
            <div key={i} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: '12px', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ color: '#ccc', fontSize: '0.9rem' }}>📍 {arici.il} &nbsp;|&nbsp; ⏳ {arici.deneyim_yili} yıl deneyim &nbsp;|&nbsp; 🐝 {arici.ari_turleri}</p>
                {kabul && <p style={{ color: '#4CAF50', fontSize: '0.85rem', marginTop: '0.5rem' }}>✅ Mentor bağlantısı kuruldu — {arici.email}</p>}
              </div>
              <button
